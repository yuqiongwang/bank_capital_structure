...
params.underline_y = 1.02;   % threshold. put 1 to recover the old result
...
plot(y, upper_bound, 'k--', 'LineWidth', 1.5, 'DisplayName', 'Upper bound: y+K');
xline(sol_qvi.y_star, 'b-', 'LineWidth', 1, 'DisplayName', sprintf('y^*=%.3f', sol_qvi.y_star));
xlabel('y = X/L');
ylabel('v(y)');
title('Value Function v(y)');
legend('Location', 'northwest');
xlim([params.underline_y + 0.01, settings.y_max]);
...
hold on;
xlim([params.underline_y + 0.01, settings.y_max]);
xline(params.underline_y, '--', '\underline y');
...
for i = 1:size(Y_mesh, 1)
    for j = 1:size(Y_mesh, 2)
        y_val = Y_mesh(i,j);
        % underline_y <= y <= y* : show only the continuation region
        if y_val >= params.underline_y && y_val <= y_star_val
...
hold on;
plot(params.underline_y * L_line, L_line, 'r-', 'LineWidth', 2);  % y=underline_y line
plot(y_star_val * L_line, L_line, 'g-', 'LineWidth', 2);  % y=y* line
xlabel('X ');
ylabel('L ');
title('v(X,L) ');
legend('', ...
       sprintf('$y=\\underline{y} = %.2f$', params.underline_y), ...
       sprintf('$y=y^*=%.2f$', y_star_val), ...
       'Location', 'northwest', 'Interpreter', 'latex');
colorbar;
%
%% save for a2_trajectory_simulation.m
results = struct();
results.params   = params;
results.settings = settings;
results.bounds   = bounds;
results.sol_qvi  = sol_qvi;
results.sol_hjb  = sol_hjb;
results.metrics  = metrics;
save('vi_solution.mat', 'results');
